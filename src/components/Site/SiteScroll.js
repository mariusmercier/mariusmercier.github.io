  ...
  useEffect(() => {
    ...
    if (initialAnchor && initialAnchor !== 'top') {
      ...
      setTimeout(() => {
        ...
        if (!target) return;
        const offset = window.innerWidth < MOBILE_BREAKPOINT ? NAV_OFFSET_MOBILE : NAV_OFFSET_DESKTOP;
        const top = target.getBoundingClientRect().top + window.scrollY - offset;
...
function TopNav({ onJump, isMobile, open, setOpen, anchor }) {
  return (
    <div style={{
...
function Sidebar({ anchor, onJump, lastUpdated }) {
  return (
...
function SectionHeader({ id, n, title, note }) {
  return (
    <div data-sec={id} style={{
      marginBottom: 28,
      paddingTop: 64,
      borderTop: `0.5px solid ${COLORS.line}`,
    }}>
...
function About({ aboutMarkdown, wordCount }) {
  return (
    <>
...
function PubRow({ entry, id }) {
  const { n, year, authors, title, venue, note, links } = entry;
...
function ContactLine({ label, href, display }) {
  const content = (
    <>
